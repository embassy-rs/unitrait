...
unitrait::unitrait! {
    ...
    pub trait Registry {
        ...
        pub fn registry_open(id: u32) -> Self::Handle;

        #[symbol = "_unitrait_test_registry_id"]
        pub fn registry_id(h: &Self::Handle) -> u32;

        #[symbol = "_unitrait_test_registry_close"]
        pub fn registry_close(h: Self::Handle) -> u32;
    }
...
#[test]
fn test_round_trip() {
    let h = registry_open(7);
    assert_eq!(registry_id(&h), 7);
    assert_eq!(registry_close(h), 7);

    // Dropping one without handing it back is a no-op, not a leak.
    let h = registry_open(9);
    assert_eq!(registry_id(&h), 9);
    drop(h);
}
